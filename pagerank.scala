...
object PageRank {
     ...
     */
    def init_rank(url_map : Map[String, List[String]])
        : Map[String, Float] = {

        // Set the initial PageRank value as 1
        url_map.map{case (url, neighbors) => (url, 1F)}
    ...
    def page_ranking(url_map  : Map[String, List[String]],
        ...
        for (i <- 0 to iter ) {
            ...
            new_rank_map = contribs_list.groupBy{ case (t0, t1) => t0 }.
                map{ case (k, group) => 
    ...
    def process(input : List[String]) {
        ...
        val url_neighbors_map = url_map(input)
        
        print(url_neighbors_map)
        
        val page_rank_map = init_rank(url_neighbors_map)
